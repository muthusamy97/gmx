...
class WPPopups_Updater {
	...
	public function __construct( array $config ) {
		...
		$this->cache_key                = 'wpp_sl_' . md5( serialize( $this->plugin_slug . $this->key . $this->beta ) );
		$edd_plugin_data[ $this->plugin_slug ] = $this->api_data;
		/**
	...
	public function init() {
		...
		remove_action( 'after_plugin_row_' . $this->plugin_name, 'wp_plugin_update_row', 10 );
	...
	public function check_update( $_transient_data ) {
		...
		if ( ! empty( $_transient_data->response ) && ! empty( $_transient_data->response[ $this->plugin_name ] ) && false === $this->wp_override ) {
			return $_transient_data;
		}
		...
		if ( false !== $version_info && is_object( $version_info ) && isset( $version_info->new_version ) ) {
			...
			$_transient_data->checked[ $this->plugin_name ] = $this->version;
		}
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( empty( $update_cache->response ) || empty( $update_cache->response[ $this->plugin_name ] ) ) {
			...
			$update_cache->checked[ $this->plugin_name ] = $this->version;
		...
		// Restore our filter
		add_filter( 'pre_set_site_transient_update_plugins', [ $this, 'check_update' ] );
		if ( ! empty( $update_cache->response[ $this->plugin_name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
	...
	public function plugins_api_filter( $_data, $_action = '', $_args = null ) {
		...
		$cache_key = 'wpp_api_request_' . md5( serialize( $this->plugin_slug . $this->api_data['key'] . $this->beta ) );
		// Get the transient where we store the api request for this plugin for 24 hours
		$wpp_api_request_transient = $this->get_cached_version_info( $cache_key );
		//If we have no transient-saved value, run the API, set a fresh transient with the API value, and return that value too right now.
		if ( empty( $wpp_api_request_transient ) ) {
			...
			$_data = $wpp_api_request_transient;
		}
	...
	private function api_request( $_action, $_data ) {
		...
		$store_hash = md5( $this->remote_url );
		if ( ! is_array( $edd_plugin_url_available ) || ! isset( $edd_plugin_url_available[ $store_hash ] ) ) {
			$test_url_parts = parse_url( $this->remote_url );
		...
		if ( $data['plugin_slug'] != $this->plugin_slug ) {
			return;
		}
		if ( $this->remote_url == trailingslashit( home_url() ) ) {
			return false; // Don't allow a plugin to ping itself
		}
		$api_params = [
			'edd_action' => 'get_version',
			'license'    => ! empty( $data['key'] ) ? $data['key'] : '',
		...
		$verify_ssl = $this->verify_ssl();
		$request    = wp_remote_post( $this->remote_url, [
			'timeout'   => 15,
	...
	public function show_changelog() {
		...
		$cache_key    = md5( 'wpp_plugin_' . sanitize_key( $_REQUEST['plugin'] ) . '_' . $beta . '_version_info' );
