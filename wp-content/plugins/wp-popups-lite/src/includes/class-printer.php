...
class WPPopups_Printer {
	...
	* set on wp_head hook
	...
	public function __construct() {
		...
		add_action( 'wp_enqueue_scripts', [ $this, 'assets_js' ] );
	...
	*/
	...
	public function head( $popup_data, $popup ) {
		...
		printf(
			'<div class="%s" id="spu-%d" data-id="%d" data-parent="%d" data-settings="%s">',
			esc_attr( wppopups_sanitize_classes( apply_filters( 'wppopups_popup_class', $classes, $popup ), true ) ),
			esc_attr( $popup->id ),
			esc_attr( $popup->id ),
	...
	public function assets_js() {
		...
		wp_localize_script( 'wppopups', 'wppopups_vars',
			[
	 ...
	 * @return string
	 */
	private function sanitizeAlign( $align ) {
