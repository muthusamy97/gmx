...
(function ($) {
    ...
    const WPPopupsBuilder = {
        ...
        init: function () {
            ...
            // Document ready
            $(document).ready(WPPopupsBuilder.ready);
        ...
        load: function () {
            ...
            $('#wppopups-builder-overlay').fadeOut();
        ...
        ready: function () {
            ...
            $builder = $('#wppopups-builder');

            // Bind all actions.
            WPPopupsBuilder.bindUIActions();

            // Trigger initial save for new popups
            ...
            s.templateList = new List(
                'wppopups-setup-templates-additional', {
            ...
            WPPopupsBuilder.trimPopupTitle();

            // Load Tooltips
            WPPopupsBuilder.loadTooltips();

            // Load ColorPickers
            WPPopupsBuilder.loadColorPickers();

            // Choices for select fields
            WPPopupsBuilder.initChoicesJS();
            ...
            $('[data-depend][data-depend!=""]').each(function (e) {
                ...
                const depend_value = $(this).data('depend-value'),
                ...
                dependent_field.on('keyup mouseup change', function () {
                    ...
                    if( dependent_field.is(':checkbox') ) {
                        if( dependent_field.is(':checked') )
                            current_value = dependent_field.val();
                        else
                            current_value = 0;
                    } else
                        current_value = dependent_field.val();
                    ...
                    if ( depend_value.toString().indexOf(current_value) != -1 )
        ...
        alterClass: function (element, removals, additions) {
        ...
        bindUIActions: function () {
            ...
            // General Panels
            WPPopupsBuilder.bindUIActionsPanels();

            // Setup Panel
            WPPopupsBuilder.bindUIActionsSetup();

            // Content Panel
            WPPopupsBuilder.bindUIActionsContent();

            // Appearance Panel
            WPPopupsBuilder.bindUIActionsAppearance();

            // Settings Panel
            WPPopupsBuilder.bindUIActionsSettings();

            // Rules
            WPPopupsBuilder.bindUIActionsRules();

            // Optin Forms
            WPPopupsBuilder.optinUIActions();

            // Providers
            WPPopupsBuilder.providersUIActions();
         ...
         */
        optinNameField: function() {
            const $placeholder = $('#wppopups-panel-field-fields-name_field_text').val();
         ...
         */
        gdprField: function() {
            const $url = $('#wppopups-panel-field-fields-gdpr_url').val();
            let $text = $('#wppopups-panel-field-fields-gdpr_field_text').val();
            if( $url.length ) {
                    ...
                    confirm: {
                        ...
                        action: function() {
                            ...
                            if (input.val() === '') {
                                ...
                                // Disable button.
                                WPPopupsBuilder.inputToggle($this, 'disable');
                                ...
                                WPPopupsBuilder.fireAJAX($this, data, function(res) {
                                    if (res.success) {
                                        $connections.find('.wppopups-provider-connections').prepend(res.data.html);
        ...
        accountAdd: function(el, e) {
            ...
            var $this       = $(el),
                ...
                errors      = WPPopupsBuilder.requiredCheck($fields, $container);
        ...
        accountSelect: function(el, e) {
            ...
            if (!$this.val()) {
                ...
                $connection.find('.wppopups-provider-account-add').slideUp();
         ...
         * @since 2.0.0
         */
        providerPanelConfirm: function(targetPanel, currentPanel) {
                            ...
                            cancel: {
                                ...
                                action: function () {
                                    if( currentPanel === 'providers' ) {
                                        const $panel = $('#wppopups-panel-' + currentPanel),
        ...
        bindUIActionsPanels: function () {
            ...
            // Panel switching
            $builder.on(
                'click', '#wppopups-panels-toggle button, .wppopups-panel-switch', function (e) {
                    e.preventDefault();
            ...
            $builder.on(
                'click', '.wppopups-panel .wppopups-panel-sidebar-section:not(".not-clickable")', function (e) {
                    WPPopupsBuilder.panelSectionSwitch(this, e);
                }
            );

            $builder.on(
                'click', '.wppopups-panel .wppopups-panel-content-wrap .back-section', function (e) {
        ...
        panelSwitch: function (panel) {
            ...
            const $panel = $('#wppopups-panel-' + panel),
            ...
            if ( ! $panel.hasClass('active') && ! $panelBtn.hasClass('upgrade-modal') ) {
                ...
                const active_panel = $('#wppopups-panels-toggle .active').data('panel');
         ...
         * @since 2.0.0
         */
        panelSectionBack: function (el, e) {
            e.preventDefault();
        ...
        bindUIActionsSetup: function () {
            ...
            $builder.on(
                'click', '.wppopups-trigger-blank', function (e) {
                    e.preventDefault();
            ...
            $builder.on(
                'input ', '#wppopups-panel-field-settings-popup_title', function () {
                    $('#wppopups-setup-name').val($('#wppopups-panel-field-settings-popup_title').val());
                }
            );
            $builder.on(
                'input', '#wppopups-setup-name', function () {
            ...
            $builder.on(
                'keyup', '#wppopups-setup-template-search', function () {
        ...
        templateSelect: function (el, e) {
            ...
            $this.html(s.spinner + ' ' + wppopups_builder_vars.loading);
                        ...
                        buttons: {
                            confirm: {
                                ...
                                action: function () {
                                    ...
                                    $.post(
                                        wppopups_builder_vars.ajax_url, data, function (res) {
                                            if (res.success) {
        ...
        // Appearance Panel
        // --------------------------------------------------------------------//
        bindUIActionsAppearance: function () {
            // Box position
            ...
            $('#wppopups-panel-field-animation-animation').on('change', function (e) {
                const class_name = 'spu-animation-' + $(this).val();
            ...
            $('#wppopups-panel-field-colors-overlay_blur').on('change', function (e) {
                ...
                const $box_css = $('#wppopups-builder .spu-bg'),
                    blur_px = $(this).val() ? 'blur('+$(this).val()+'px)' : 'blur(2px)';

                $box_css.css('filter',blur_px)
                    .css('webkitFilter',blur_px)
                    .css('mozFilter',blur_px)
                    .css('oFilter',blur_px)
                    .css('msFilter',blur_px);
        ...
        // Content Panel
        // --------------------------------------------------------------------//
        bindUIActionsContent: function () {
            if (typeof window.parent.tinymce !== 'undefined') {
                window.parent.tinymce.on(
        ...
        updateEditorContent: function(editor) {
            const fields_container = $('.spu-content').find('.spu-fields-container');
            const addon_container = $('.spu-content').find('.spu-addon-container');
        ...
        bindUIActionsSettings: function () {
            ...
            $builder.on(
                'click', '.wppopups-title-desc, .wppopups-field-submit-button, .wppopups-center-popup-name', function (e) {
                    e.preventDefault();
            ...
            $builder.on(
                'input', '#wppopups-panel-field-settings-popup_title, #wppopups-setup-name', function () {
                    var title = $(this).val();
                    if (title.length > 38) {
            ...
            // change trigger
            $builder.on(
                'change', '.trigger-option .choicesjs-select', function () {
            ...
            // Delete row
            $builder.on(
                'click', '.trigger-actions .remove', function (e) {
                    e.preventDefault();
            ...
            // Add row
            $builder.on(
                'click', '.trigger-actions .add', function (e) {
                    e.preventDefault();
         ...
         * @since 2.0.0
         */
        triggersChangeTrigger: function (el) {
            ...
            this.renderRulesLoading($row, true);
         ...
         * @since 2.0.0
         */
        triggersDeleteRow: function (e, el) {

            var $this = $(el),
                $row = $this.closest('div.trigger-tr'),
                $table = $this.closest('div.trigger-group'),
                total = $table.find('div.trigger-tr').length;

            // Delete only if one more than row exist
            if (total > '1') {
                $row.remove();
            }
        },
        ...
        triggersClone: function (el) {
            ...
            $new_row.find('[name]').each(function () {
                // update names
                $(this).attr('name', $(this).attr('name').replace('trigger_id', row_new_id));
                // update ids
                $(this).attr('id', $(this).attr('id').replace('trigger_id', row_new_id));
                // update classes
                $(this).addClass('choicesjs-select');
        ...
        bindUIActionsSaveExit: function () {
            ...
            // Save popup
            $builder.on(
                'click', '#wppopups-save', function (e) {
                    e.preventDefault();
            ...
            // Publish popup
            $builder.on(
                'click', '#wppopups-publish', function (e) {
                    e.preventDefault();
            ...
            // Exit builder
            $builder.on(
                'click', '#wppopups-exit', function (e) {
                    e.preventDefault();
         ...
         * @since 2.0.0
         */
        popupSave: function (redirect, publish) {
         ...
         * @since 2.0.0
         */
        bindUIActionsRules: function () {

            // change rule
            $builder.on(
                'change', '.rule-option .choicesjs-select', function () {
            ...
            // Delete row
            $builder.on(
                'click', '.rule-actions .remove', function (e) {
                    e.preventDefault();
            ...
            // Add row
            $builder.on(
                'click', '.rule-actions .add', function (e) {
                    e.preventDefault();
         ...
         * @param $row
         */
        renderRulesLoading: function ($row, disable) {
            const spinner = ' <i class="fa fa-spinner fa-spin wppopups-loading-inline"></i>';
            if (disable) {
         ...
         * @since 2.0.0
         */
        rulesClone: function (el, cloning) {

            var $this = $(el),
                $row = $this.closest('div.rule-tr'),
                $group = $row.closest('div.rule-group');

            // if we are cloning group (OR)
            if ('group' == cloning) {
            ...
            $new_row.find('[name]').each(function () {
                ...
                $(this).attr('id', $(this).attr('id').replace('group_id', group_new_id));
                // update classes
                $(this).addClass('choicesjs-select');
                // remove disabled attr
                $(this).removeAttr('disabled');
        ...
        requiredCheck: function(fields, location) {
            ...
            location.find('.wppopups-alert-required').remove();
