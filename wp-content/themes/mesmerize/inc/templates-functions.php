...
function mesmerize_print_logo($footer = false)
{
    ...
    if (function_exists('has_custom_logo') && has_custom_logo()) {
        ...
        if ($dark_logo_image) {
            $dark_logo_html = sprintf('<a href="%1$s" class="logo-link dark" rel="home" itemprop="url"  data-type="group" ' . $preview_atts . ' data-dynamic-mod="true">%2$s</a>', esc_url(home_url('/')), wp_get_attachment_image(absint($dark_logo_image), 'full', false, array(
                'class'    => 'logo dark',
                'itemprop' => 'logo',
            )));
...
{
    $copyrightText = __('Copyright 2020 GyanMatrix Technologies Pvt. Ltd. <a  class="mesmerize-theme-link"></a>', 'mesmerize');
